...
if args.setting == "stationary":
    T_list = [100, 110, 120, 130, 140, 150]
    for T in T_list:
        RESULT_FILE_PATH.append(os.path.join(CUR_DIR, foldername, "framesize_" + str(T) + ".npy"))
elif args.setting == "aperiodic":
    T_list = [15, 20, 25, 30, 35, 40, 45, 50]
    for T in T_list:
        RESULT_FILE_PATH.append(os.path.join(CUR_DIR, foldername, "framesize_" + str(T) + ".npy"))
...
FIGURE_FILE_PATH = os.path.join(CUR_DIR, foldername, args.setting + "_diff_frame_size" + ".pdf")
...
with sns.plotting_context("notebook"):
    ...
    new_title = 'Frame size'
    fig._legend.set_title(new_title)
    fig.set_axis_labels('Time', 'Total Queue Length', labelpad=10)
    if args.setting == "stationary":
        sns.move_legend(fig, loc="upper left", bbox_to_anchor=(0.128, 0.96), frameon=True)
        fig.set(ylim=(0, 1400))
        fig.set(xlim=(0, time_horizon))
        plt.xticks([0, 20000, 40000, 60000, 80000, 100000],
                   ['0', '20k', '40k', '60k', '80k', '100k'])
    elif args.setting == "aperiodic":
        sns.move_legend(fig, loc="upper left", bbox_to_anchor=(0.716, 0.97), frameon=True)
        fig.set(ylim=(0, 400))
        fig.set(xlim=(0, time_horizon))
        ...
        sns.move_legend(fig, loc="upper left", bbox_to_anchor=(0.714, 0.96), frameon=True)
        fig.set(ylim=(0, 400))
        fig.set(xlim=(0, time_horizon))
